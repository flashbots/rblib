...
/// - If any shared signer reuses (signer, nonce) between the two orders or the
///   per‑signer nonce intervals overlap -> `MutuallyExclusive`.
...
///   earlier.max + 1).
/// - Distant* variants arise when at least one shared signer has a positive gap
///   (later.min > earlier.max + 1); missing nonces must be filled by other txs.
///
/// Invariants / Assumptions:
/// - Within a single order, for a given signer, nonces are guaranteed elsewhere
///   to be strictly increasing without internal gaps; therefore each signer’s
///   nonce usage can be summarized by a closed interval [min, max].
pub fn nonce_relation<P: Platform>(
	left: &Order<P>,
	right: &Order<P>,
) -> NonceRelation {
	#[derive(Clone, Copy)]
...
mod tests {
	...
	>() {
		...
		assert_eq!(nonce_relation(&a, &b), NonceRelation::Independent);
		assert_eq!(nonce_relation(&b, &a), NonceRelation::Independent);
	}
	...
	fn mutually_exclusive_overlap_single<P: TestablePlatform>() {
		...
		assert_eq!(nonce_relation(&a, &b), NonceRelation::MutuallyExclusive);
		assert_eq!(nonce_relation(&b, &a), NonceRelation::MutuallyExclusive);
	}
	...
	fn direct_ancestor_single_signer<P: TestablePlatform>() {
		...
		assert_eq!(nonce_relation(&a, &b), NonceRelation::DirectAncestor);
		assert_eq!(nonce_relation(&b, &a), NonceRelation::DirectDescendant);
	}
	...
	fn distant_ancestor_single_signer<P: TestablePlatform>() {
		...
		assert_eq!(nonce_relation(&a, &b), NonceRelation::DistantAncestor);
		assert_eq!(nonce_relation(&b, &a), NonceRelation::DistantDescendant);
	}
	...
	fn direct_descendant_single_signer<P: TestablePlatform>() {
		...
		assert_eq!(
			nonce_relation(&later, &earlier),
			NonceRelation::DirectDescendant
		);
		assert_eq!(
			nonce_relation(&earlier, &later),
			NonceRelation::DirectAncestor
		);
	}
